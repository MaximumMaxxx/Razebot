...
class Rankcheck(commands.Cog):
    ...
    def get_acc(self, account: str) -> discord.Embed:
        ...
        try:
            uuid = requests.get(
                f"https://api.henrikdev.xyz/valorant/v1/account/{acc}/{tag}").json()['puuid']
        except discord.ApplicationCommandInvokeError as exception:
            logging.exception(
        ...
        if MH.status_code == 200:
            ...
            embed.add_field(name="Stats from the last 5 gmaes",
    ...
    async def rclist(self, ctx: discord.ApplicationContext, list: str = option(name="List", description="The list that you want to pull the accounts from. (my | quick)", Required=True)):
        ...
        await msg.edit(embed=self.get_acc(accounts[account_index]))

    @commands.slash_command(name="rankcheckaccount", description="Get the stats for a specific VALORANT account")
    async def rcacc(self, ctx: discord.ApplicationContext, account: str = option(name="Account", description="The VALORANT account you would like to check the rank of"), region: str = option(name="Region", description=f"The region the account is in. If not specified will default to the server's default region.", choices=["Na", "Eu"])):
        ...
        await ctx.respond("Working on that ... please wait")
        msg = await ctx.interaction.original_message()
        await msg.edit(embed=self.get_acc((None, account.split("#")[0], region,  account.split("#")[1])))
