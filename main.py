...
async def rc(ctx):  
    try:  
        ...
        if MMR.status_code==200:
            ...
            item = discord.Embed(title=name.content, description=f"The stats and rank for {name.content}" ,color=EMB_color)
            item.add_field(name="Rank",value=rank,inline=True)
            item.add_field(name="MMR",value=elo,inline=True)
        else:
            item = discord.Embed(title=name.content, description=output, color=EMB_color)
